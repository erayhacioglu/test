...
const UserBackground = () => {
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      ...
      Axios.post(`/user/update-banner-img`).then(res => {
        console.log('res', res)
      }).catch(err => {
        const msg = err?.response?.data?.message || "Arka Plan Resmi Değiştirilemedi"
        toast.error(msg);
      })
    }
  };
  ...
  return (
    <div className={`user_background ${isUpdated ? "update_mode" : ""}`}>
      <img src={userImagesState?.bannerImg ?? defaultBackground} alt="User background" className="user_background_img" />
      {isUpdated && (
