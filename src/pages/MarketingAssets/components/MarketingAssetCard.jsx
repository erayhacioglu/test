...
const MarketingAssetCard = ({ isUpdated, data }) => {
  ...
  return (
    <div
      className={`marketing_assets_card_container ${isUpdated ? "update" : ""}`}
    >
      <div
        className="marketing_assets_card"
        onClick={() => handleClickCard(data?.url)}
      >
        ...
        <div className="marketing_assets_card_content">
          <p className="marketing_assets_card_content_text">{data?.name}</p>
        </div>
      </div>
      <button
        className="marketing_assets_card_delete_button"
        onClick={() => handleDeleteItem(data?.id)}
      >
        <img
          src={trash}
          alt=""
          className="marketing_assets_card_delete_button_img"
        />
      </button>
    </div>
  );
